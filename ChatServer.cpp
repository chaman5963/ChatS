    ...
    case WM_TIMER:
        ...
        else
        {
            if (nameList.size() != 0)
            {
                ...
                {
                    if (fromAddr.sin_addr.s_addr == nameList[i].addr.sin_addr.s_addr &&
                        fromAddr.sin_port == nameList[i].addr.sin_port)
                    {
                        is = true;
                    }
                }
            ...
            for (int i = 0; i < nameList.size(); i++)
            {
                toAddr = nameList[i].addr;
                memset(&toAddr, 0, sizeof(toAddr));
                toAddr.sin_family = AF_INET;
                inet_pton(AF_INET, ipAddr, &toAddr.sin_addr.s_addr);
                toAddr.sin_port = nameList[i].addr.sin_port;
